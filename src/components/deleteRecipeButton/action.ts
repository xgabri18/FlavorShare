			...
			if (
				(
					await db
						...
						.from(usedIngredients)
						.where(
							eq(usedIngredients.ingredient_id, dbIngredient.ingredient_id)
						)
				).length === 0
			) {
				await db
					.delete(ingredients)
					.where(eq(ingredients.id, dbIngredient.ingredient_id));
			}
		});
			...
			if (
				(
					await db
						.select()
						.from(recipeCategories)
						.where(eq(recipeCategories.category_id, dbCategory.category_id))
				).length === 0
			) {
				await db
					.delete(categories)
					.where(eq(categories.id, dbCategory.category_id));
			}
		});
		await db
			.delete(restaurantRecipes)
			.where(eq(restaurantRecipes.recipe_id, recipeId));
		await db
			.delete(favoriteRecipes)
			.where(eq(favoriteRecipes.recipe_id, recipeId));
