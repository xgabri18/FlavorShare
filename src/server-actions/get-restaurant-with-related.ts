...
const getRestaurantWithRelated = async (id: number, userId: string) => {
    const restaurantRows = await db.select().from(restaurants).where(eq(restaurants.id, Number(id)));
    if (restaurantRows.length !== 1 || restaurantRows[0].user_id !== userId)
    {
        return null;
    }
    const restaurant = restaurantRows[0];
    const cheffRows = await db.select(
        {name: users.name}
    ).from(
        users
    ).where(
        eq(users.restaurant_id, restaurant.id)
    );
