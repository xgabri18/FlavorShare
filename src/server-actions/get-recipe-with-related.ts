...
const getRecipeWithRelated = async (id: number, userId: string) => {
    const recipeRows = await db.select().from(recipes).where(eq(recipes.id, Number(id)));
    ...
    const categoryRows = await db.select(
        ...
        eq(recipeCategories.recipe_id, recipe.id)
    );
    const ingredientRows = await db.select({
        ...
        eq(usedIngredients.recipe_id, recipe.id)
    );
