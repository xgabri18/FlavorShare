...
export const addRestaurant = async ({cheffs, userId, ...data}: RestaurantForm & {userId: string}) => {
    await checkCheffsExist(cheffs);
    await checkRestaurantName(data.name);
	const result = (await db.insert(restaurants).values({...data, user_id: userId}).returning({id: restaurants.id}));
    if (result.length !== 1) {
        throw new Error("Unable to insert restaurant.");
    }
    cheffs.forEach(async (cheff) => {
        const count = (
            await db.update(users).set({restaurant_id: result[0].id}).where(
                and(eq(users.name, cheff.name), eq(users.chef, true), isNull(users.restaurant_id))
            )).rowsAffected;
        if (count !== 1) {
