...
const Page = async ({ params }: RecipePageProps) => {
    ...
    const restaurant = await getRestaurantWithRelated(Number(id), session.user.id);
    if (restaurant === null)
    {
        return notFound();
    }
	...
	return (
		<main className="md:container md:w-4/5">
			<div className="flex">
				<RestaurantFormComponent userId={session.user.id ?? ""} restaurantId={restaurant.id} data={restaurant}/>
			</div>
		</main>
	);
};
